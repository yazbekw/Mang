...
"""
بوت أسئلة شامل - النسخة النهائية المتكاملة
- جميع الأزرار تعمل بما فيها التصفية المتقدمة
- آلية تصويت لاختيار الإجابة الصحيحة
- دعم كامل للملفات الثمانية الموحدة
- إحصائيات متقدمة حسب الفئة والنوع
"""
...
def normalize_options(options):
    if isinstance(options, dict):
        keys = sorted(options.keys(), key=lambda x: int(x) if str(x).isdigit() else str(x))
...
def get_answer_index(options, correct_answer):
    if isinstance(options, dict):
        ...
        try:
            numeric_key = str(int(correct_answer))
            if numeric_key in keys:
                return keys.index(numeric_key)
        except:
            pass
        for idx, key in enumerate(keys):
            if str(correct_answer).strip() == str(options[key]).strip():
                return idx
        return 0
    else:
        # الخيارات قائمة (list)
...
def load_questions_from_json():
    ...
    for file in files:
        try:
            with open(file, 'r', encoding='utf-8') as f:
                data = json.load(f)
            file_category = data.get('category', 'غير مصنف')
            raw = []
            ...
            for q in raw:
                ...
                if q.get('type') == 'truefalse':
                    options_list = ["صحيح", "خطأ"]
                    correct_val = q.get('correct')
                    if isinstance(correct_val, bool):
                        answer_idx = 0 if correct_val else 1
                    else:
                        answer_idx = 0
                else:
                    ...
                    answer_idx = get_answer_index(options_raw, correct_val)
                ...
                if question_text not in all_versions:
                    all_versions[question_text] = []
                all_versions[question_text].append(version)
    ...
    for question_text, versions in all_versions.items():
        if len(versions) == 1:
            final_questions.append(versions[0])
        else:
            answer_votes = {}
            for v in versions:
                ans = v['answer']
            ...
            most_common_answers = [ans for ans, count in answer_votes.items() if count == max_votes]
            if len(most_common_answers) == 1:
                winning_answer = most_common_answers[0]
                candidates = [v for v in versions if v['answer'] == winning_answer]
                best_version = max(candidates, key=lambda x: x['num_options'])
            else:
                best_version = max(versions, key=lambda x: (x['num_options'], x['source_file']))
            final_questions.append(best_version)
            if len(versions) > 1:
                duplicates_skipped += (len(versions) - 1)
                logger.info(f"🔍 تم حل تعارض للسؤال: {question_text[:50]}... باستخدام الإجابة من {best_version['source_file']}")

    count = 0
    for q in final_questions:
        c.execute(
            "INSERT INTO questions (question, options, answer, explanation, category) VALUES (?, ?, ?, ?, ?)",
            (q['question'], json.dumps(q['options']), q['answer'], q['explanation'], q['category'])
        )
        count += 1

    conn.commit()
    conn.close()
    logger.info(f"✅ تم تحميل {count} سؤال (تم تخطي {duplicates_skipped} نسخة مكررة)")
    return count
...
def build_main_menu(user_id=None):
    buttons = [
        ...
        [InlineKeyboardButton("🎯 تصفية متقدمة (فئة + نوع)", callback_data="advanced_filter")],
...
def build_question_keyboard(qid, idx, total, state, time_left=None):
    ...
    buttons.append(nav)
    buttons.append([InlineKeyboardButton("🔀 خلط الأسئلة", callback_data="shuffle_current")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "✨ <b>المزايا:</b>\n"
        "• اختبارات عشوائية أو حسب الفئة والنوع\n"
        "• اختبار مخصص بعدد أسئلة محدد مع مؤقت\n"
        "• وضع التعلم لعرض السؤال مع الإجابة والشرح\n"
        "• إشارات مرجعية وتتبع الأخطاء\n"
...
async def custom_quiz_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "مثال: <code>10 5</code> (10 أسئلة و 5 دقائق)\n\n"
...
async def handle_quiz_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = text.split()
    if len(parts) != 2:
        await update.message.reply_text("❌ استخدم: <code>10 5</code>", parse_mode=ParseMode.HTML)
        return
    try:
        count = int(parts[0])
        minutes = int(parts[1])
    ...
    if count <= 0 or minutes <= 0:
        await update.message.reply_text("❌ الأرقام يجب أن تكون أكبر من صفر.")
        return
    context.user_data['awaiting_custom_quiz'] = False
    available_qids = get_unanswered_questions(user_id)
    if not available_qids:
        await update.message.reply_text("⚠️ لا توجد أسئلة متاحة.", reply_markup=build_main_menu(user_id))
        return
    ...
    save_user_state(user_id, state)
    job = context.job_queue.run_once(quiz_timeout, minutes * 60, user_id=user_id, name=f"quiz_timeout_{user_id}")
    context.user_data['timeout_job'] = job
    await update.message.reply_text(f"⏱ بدء الاختبار: {count} سؤالاً، {minutes} دقيقة. حظاً موفقاً!", parse_mode=ParseMode.HTML)
    await show_current_question(update, context, user_id)
...
async def show_current_question(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id=None):
    ...
    combined_keyboard = InlineKeyboardMarkup(option_keyboard.inline_keyboard + nav_keyboard.inline_keyboard)
    try:
        if update.callback_query:
            await update.callback_query.edit_message_text(header_text, parse_mode=ParseMode.HTML, reply_markup=combined_keyboard)
        else:
            await update.message.reply_text(header_text, parse_mode=ParseMode.HTML, reply_markup=combined_keyboard)
    except Exception as e:
...
async def send_quiz_complete(update, context, user_id):
    ...
    for qid_str, opt in state['answers'].items():
        ...
        if q and q[3] == opt:
            correct += 1
    text = f"🎉 <b>انتهى الاختبار!</b>\n📊 ✅ صحيح: {correct}\n❌ خطأ: {answered - correct}\n📝 تم الإجابة على {answered}/{total}"
    if update.callback_query:
...
async def quiz_timeout(context: ContextTypes.DEFAULT_TYPE, user_id=None):
    ...
    await context.bot.send_message(user_id, text, parse_mode=ParseMode.HTML, reply_markup=build_main_menu(user_id))
    state['quiz_time'] = None
    state['start_time'] = None
    save_user_state(user_id, state)
...
async def explain_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("show_explain_"):
        ...
        if q:
            ...
            text = f"📖 <b>الشرح:</b>\n{expl}"
            await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 رجوع", callback_data=f"back_from_explain_{qid}")]]))
...
async def shuffle_current(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    state = get_user_state(user_id)
    qids = state['current_ids']
    if len(qids) < 2:
        await query.answer("يجب أن يكون هناك سؤالان على الأقل.")
        return
    random.shuffle(qids)
    state['current_ids'] = qids
    state['current_index'] = 0
...
async def categories(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text("📂 <b>اختر فئة:</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(buttons))
...
# ======================== التصفية المتقدمة ========================
async def advanced_filter_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    if 'filter_data' not in context.user_data:
        context.user_data['filter_data'] = {'category': None, 'qtype': None}
    filter_data = context.user_data['filter_data']
    categories_list = get_categories()
    buttons = []
    buttons.append([InlineKeyboardButton("📂 اختر الفئة:", callback_data="noop")])
    for cat in categories_list:
        selected = "✅ " if filter_data['category'] == cat else ""
        buttons.append([InlineKeyboardButton(f"{selected}{cat}", callback_data=f"filter_set_cat_{cat}")])
    buttons.append([InlineKeyboardButton(f"{'✅ ' if filter_data['category'] is None else ''}جميع الفئات", callback_data="filter_set_cat_None")])
    buttons.append([InlineKeyboardButton("", callback_data="noop")])
    buttons.append([InlineKeyboardButton("🎯 اختر النوع:", callback_data="noop")])
    type_options = [("جميع الأنواع", None), ("اختيار من متعدد", "mcq"), ("صح/خطأ", "truefalse")]
    for label, value in type_options:
        selected = "✅ " if filter_data['qtype'] == value else ""
        buttons.append([InlineKeyboardButton(f"{selected}{label}", callback_data=f"filter_set_type_{value if value else 'None'}")])
    buttons.append([InlineKeyboardButton("", callback_data="noop")])
    buttons.append([InlineKeyboardButton("✅ تطبيق", callback_data="filter_apply"), InlineKeyboardButton("🗑 إلغاء", callback_data="filter_cancel")])
    buttons.append([InlineKeyboardButton("🔙 رجوع", callback_data="menu")])
    await query.edit_message_text(
        "🎯 <b>تصفية متقدمة</b>\nاختر الفئة ونوع الأسئلة.\n(انقر على الخيار لتحديده/إلغائه)",
        parse_mode=ParseMode.HTML,
        reply_markup=InlineKeyboardMarkup(buttons)
    )

async def filter_set_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    cat = query.data.split("_")[3]
    if cat == 'None':
        cat = None
    context.user_data['filter_data']['category'] = cat
    await advanced_filter_start(update, context)

async def filter_set_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    qtype = query.data.split("_")[3]
    if qtype == 'None':
        qtype = None
    context.user_data['filter_data']['qtype'] = qtype
    await advanced_filter_start(update, context)
...
async def filter_apply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    user_id = query.from_user.id
    filter_data = context.user_data.get('filter_data', {})
    category = filter_data.get('category')
    qtype = filter_data.get('qtype')
    conn = get_db_connection()
    c = conn.cursor()
    sql = "SELECT id, question, options, answer, explanation, category FROM questions WHERE 1=1"
    params = []
    if category:
        sql += " AND category = ?"
        params.append(category)
    if qtype == 'truefalse':
        sql += " AND json_array_length(options) = 2"
    elif qtype == 'mcq':
        sql += " AND json_array_length(options) > 2"
    ...
    if not rows:
        await query.edit_message_text(
            "⚠️ <b>لا توجد أسئلة تطابق التصفية.</b>",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 العودة للتصفية", callback_data="advanced_filter")], [InlineKeyboardButton("🏠 القائمة", callback_data="menu")]])
        )
        return
    questions = []
    for row in rows:
        options = normalize_options(json.loads(row[2]))
        questions.append((row[0], row[1], options, row[3], row[4], row[5]))
    state = get_user_state(user_id)
    state['current_ids'] = [q[0] for q in questions]
    state['current_index'] = 0
    state['answers'] = {}
    state['mode'] = 'normal'
    save_user_state(user_id, state)
    await show_current_question(update, context, user_id)
    await query.message.reply_text(f"✅ تم العثور على {len(questions)} سؤالاً.", parse_mode=ParseMode.HTML)

async def filter_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data.pop('filter_data', None)
...
async def bookmarks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    user_id = query.from_user.id
    bookmarks_ids = get_bookmarks(user_id)
    if not bookmarks_ids:
        await query.edit_message_text("⭐ لا توجد إشارات مرجعية.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
        return
    qs = [q for qid in bookmarks_ids if (q := get_question_by_id(qid))]
    if not qs:
        await query.edit_message_text("⚠️ بعض الإشارات غير صالحة.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
        return
    state = get_user_state(user_id)
    state['current_ids'] = [q[0] for q in qs]
    state['current_index'] = 0
...
async def wrong_only(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not wrong_ids:
        await query.edit_message_text("🥳 <b>لا توجد أخطاء! أحسنت!</b>", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
        return
    qs = [q for qid in wrong_ids if (q := get_question_by_id(qid))]
    if not qs:
        await query.edit_message_text("⚠️ لا توجد أسئلة خاطئة صالحة.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
        return
    state = get_user_state(user_id)
    state['current_ids'] = [q[0] for q in qs]
    state['current_index'] = 0
...
async def suggest_questions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = query.from_user.id
    state = get_user_state(user_id)
    # بسيط: اقتراح 10 أسئلة من فئات عشوائية لم يتم الإجابة عليها بعد
    all_q = get_all_questions()
    answered = set(get_answer_log(user_id).keys())
    used = set(state.get('used_questions', []))
    available = [q for q in all_q if q[0] not in answered and q[0] not in used]
    if not available:
        await query.edit_message_text("🎉 لا توجد أسئلة جديدة متاحة.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
        return
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [f"📊 <b>إحصائياتي</b>", f"📝 الإجمالي: {total}", f"✅ صحيح: {correct}", f"❌ خطأ: {wrong}", f"🎯 النسبة: {pct:.1f}%", "", "📂 <b>حسب الفئة:</b>"]
    ...
    text = "\n".join(lines)
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=build_back_button())
...
async def export_results(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for qid in state['current_ids']:
        ...
        writer.writerow([q[1], user_ans_text, correct_text, 'نعم' if is_correct else 'لا', q[5]])
    output.seek(0)
    await query.message.reply_document(document=output.getvalue().encode('utf-8-sig'), filename=f"results_{user_id}.csv", caption="📥 <b>نتائجك</b>")
    await query.edit_message_text("✅ تم التصدير بنجاح.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
...
async def reset(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_user_state(user_id, state)
    await query.edit_message_text("🔄 <b>تمت إعادة التعيين بنجاح.</b>", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(user_id))
...
def add_question(question, options, answer, explanation, category):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("INSERT INTO questions (question, options, answer, explanation, category) VALUES (?, ?, ?, ?, ?)", (question, json.dumps(options), answer, explanation, category))
    qid = c.lastrowid
    conn.commit()
    conn.close()
    return qid
...
def update_question(qid, question, options, answer, explanation, category):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE questions SET question=?, options=?, answer=?, explanation=?, category=? WHERE id=?", (question, json.dumps(options), answer, explanation, category, qid))
    affected = c.rowcount
    conn.commit()
    conn.close()
    return affected
...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    buttons = [
        ...
        [InlineKeyboardButton("🔍 فحص التكرارات", callback_data="admin_duplicates")],
        [InlineKeyboardButton("🔙 رجوع", callback_data="menu")],
    ]
    await query.edit_message_text("⚙️ <b>لوحة تحكم المشرف</b>", parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(buttons))

async def admin_find_duplicates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    user_id = query.from_user.id
    if user_id not in ADMIN_IDS:
    ...
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT question, COUNT(*) as cnt FROM questions GROUP BY question HAVING cnt > 1")
    rows = c.fetchall()
    conn.close()
    if not rows:
        await query.edit_message_text("✅ لا توجد أسئلة مكررة.", reply_markup=build_back_button())
        return
    text = "⚠️ الأسئلة المكررة:\n"
    for q, cnt in rows:
        text += f"• {escape_html(q[:80])}... (×{cnt})\n"
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=build_back_button())
...
async def admin_add_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await query.edit_message_text("⛔ غير مصرح.")
        return
    await query.edit_message_text("📝 <b>إضافة سؤال جديد</b>\nأدخل نص السؤال:", parse_mode=ParseMode.HTML)
...
async def admin_add_question_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📋 <b>أدخل الخيارات مفصولة بفواصل</b>\nمثال: <code>خيار1, خيار2, خيار3, خيار4</code>", parse_mode=ParseMode.HTML)
...
async def admin_add_options(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(options) < 2:
        await update.message.reply_text("⚠️ يجب أن يكون هناك خياران على الأقل.", parse_mode=ParseMode.HTML)
        return ADD_QUESTION_OPTIONS
    context.user_data['new_options'] = options
    opts = "\n".join([f"{i+1}. {opt}" for i, opt in enumerate(options)])
    await update.message.reply_text(f"✅ الخيارات:\n{opts}\n\nأدخل رقم الإجابة الصحيحة (1-{len(options)}):", parse_mode=ParseMode.HTML)
...
async def admin_add_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text(f"⚠️ رقم غير صحيح. أدخل رقم بين 1 و {len(options)}:", parse_mode=ParseMode.HTML)
        return ADD_QUESTION_ANSWER
    context.user_data['new_answer'] = ans - 1
    await update.message.reply_text("📖 <b>أدخل شرح السؤال</b> (أو أرسل <code>-</code> لتخطي):", parse_mode=ParseMode.HTML)
...
async def admin_add_explanation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("📂 <b>أدخل الفئة</b> (أو أرسل <code>-</code> للفئة الافتراضية):", parse_mode=ParseMode.HTML)
...
async def admin_add_category(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ <b>تمت الإضافة بنجاح!</b>\nرقم السؤال: <code>{qid}</code>", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(update.effective_user.id))
...
async def admin_delete_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await query.edit_message_text("⛔ غير مصرح.")
        return
    await query.edit_message_text("🗑 <b>حذف سؤال</b>\nأدخل رقم السؤال المراد حذفه:", parse_mode=ParseMode.HTML)
...
async def admin_delete_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if affected:
        await update.message.reply_text(f"✅ <b>تم حذف السؤال رقم {qid} بنجاح.</b>", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(update.effective_user.id))
    else:
        await update.message.reply_text(f"⚠️ السؤال رقم {qid} غير موجود.", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(update.effective_user.id))
    return ConversationHandler.END
...
async def admin_edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await query.edit_message_text("⛔ غير مصرح.")
        return
    await query.edit_message_text("✏️ <b>تعديل سؤال</b>\nأدخل رقم السؤال المراد تعديله:", parse_mode=ParseMode.HTML)
...
async def admin_edit_get(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (f"📌 <b>السؤال الحالي (ID: {qid})</b>\nالسؤال: {escape_html(q[1])}\nالخيارات: {', '.join(escape_html(opt) for opt in q[2])}\nالإجابة الصحيحة: {q[3]+1} - {escape_html(q[2][q[3]])}\nالشرح: {escape_html(q[4]) if q[4] else 'لا يوجد'}\nالفئة: {escape_html(q[5])}\n\nأدخل البيانات الجديدة بالصيغة:\n<code>السؤال | الخيار1,خيار2,خيار3,خيار4 | رقم_الإجابة | الشرح | الفئة</code>\n(استخدم <code>-</code> لتخطي حقل معين)")
...
async def admin_edit_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = text.split('|')
    if len(parts) != 5:
        await update.message.reply_text("⚠️ الصيغة غير صحيحة. يجب أن تحتوي على 5 حقول مفصولة بـ <code>|</code>.", parse_mode=ParseMode.HTML)
        return EDIT_QUESTION_STATE
    ...
    if affected:
        await update.message.reply_text(f"✅ <b>تم تعديل السؤال رقم {qid} بنجاح.</b>", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(update.effective_user.id))
    else:
...
async def admin_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    count = c.fetchone()[0]
    conn.close()
    await query.edit_message_text(f"📋 <b>إجمالي الأسئلة:</b> {count}\nاستخدم الأمر <code>/list_questions</code> لعرضها مع ترقيم.", parse_mode=ParseMode.HTML, reply_markup=build_back_button())
...
async def admin_import_json(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ <b>تم استيراد {count} سؤال من ملفات JSON.</b>", parse_mode=ParseMode.HTML, reply_markup=build_main_menu(user_id))
...
def main():
    ...
    admin_delete_conv = ConversationHandler(
        ...
        states={DELETE_QUESTION_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_delete_confirm)]},
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
        per_user=True,
    )
    ...
    admin_edit_conv = ConversationHandler(
        ...
        states={EDIT_QUESTION_STATE: [MessageHandler(filters.TEXT & ~filters.COMMAND, admin_edit_get)]},
        fallbacks=[CommandHandler("cancel", lambda u, c: ConversationHandler.END)],
        per_user=True,
    )
    ...
    application.add_handler(CallbackQueryHandler(shuffle_current, pattern="^shuffle_current$"))
    # معالجات القائمة
    ...
    # تصفية متقدمة
    application.add_handler(CallbackQueryHandler(advanced_filter_start, pattern="^advanced_filter$"))
    application.add_handler(CallbackQueryHandler(filter_set_category, pattern="^filter_set_cat_"))
    application.add_handler(CallbackQueryHandler(filter_set_type, pattern="^filter_set_type_"))
    application.add_handler(CallbackQueryHandler(filter_apply, pattern="^filter_apply$"))
    application.add_handler(CallbackQueryHandler(filter_cancel, pattern="^filter_cancel$"))
    # لوحة المشرف
    ...
    application.add_handler(CallbackQueryHandler(admin_find_duplicates, pattern="^admin_duplicates$"))
