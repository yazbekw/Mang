...
"""
بوت أسئلة شامل - النسخة المتطورة النهائية
- دعم كامل للتصفية المتقدمة (فئة + نوع)
- آلية تصويت لاختيار الإجابة الصحيحة في حالة التكرار
- دعم صيغ JSON المختلفة (قوائم وقواميس)
- إحصائيات متقدمة حسب الفئة والنوع
- أزرار إضافية (خلط، تصفية، إلخ)
- توافق كامل مع الملفات الثمانية الموحدة
"""
...
def get_answer_index(options, correct_answer):
    """
    تحويل الإجابة الصحيحة (التي قد تكون حرفاً أو رقماً أو منطقياً) إلى مؤشر صحيح (0-index).
...
# ======================== تحميل الأسئلة بآلية التصويت ========================
def load_questions_from_json():
    """
    تحميل الأسئلة من جميع ملفات JSON مع آلية تصويت لاختيار الإجابة الصحيحة في حالة التكرار.
    الأولوية: الأغلبية > الأكثر اكتمالاً > أحدث ملف.
    """
    ...
    # تجميع كل النسخ
    all_versions = {}  # المفتاح: نص السؤال، القيمة: قائمة من النسخ
    ...
    # اختيار النسخة الفائزة لكل سؤال
    final_questions = []
    ...
    # إدراج الأسئلة النهائية في قاعدة البيانات
    count = 0
...
def get_question_types():
    ...
    c.execute("""
        SELECT DISTINCT 
            CASE 
                WHEN json_array_length(options) = 2 THEN 'صح/خطأ'
...
def get_questions_by_type(qtype):
    conn = get_db_connection()
    c = conn.cursor()
    if qtype == 'صح/خطأ':
        c.execute("""
            SELECT id, question, options, answer, explanation, category 
            FROM questions 
            WHERE json_array_length(options) = 2
        """)
    else:  # اختيار من متعدد
        c.execute("""
            SELECT id, question, options, answer, explanation, category 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "• اختبار مخصص بعدد أسئلة محدد (بدون تكرار)\n"
...
async def custom_quiz_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        "لإضافة تصفية بالنوع أو الفئة، استخدم خيارات التصفية أولاً.",
        parse_mode=ParseMode.HTML
    )
...
async def shuffle_current(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    qids = state['current_ids']
    if len(qids) < 2:
        await query.answer("يجب أن يكون هناك سؤالان على الأقل للخلط.")
        return
    random.shuffle(qids)
    state['current_ids'] = qids
    state['current_index'] = 0
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # إحصائيات حسب النوع
    type_stats = {'اختيار من متعدد': {'correct': 0, 'wrong': 0}, 'صح/خطأ': {'correct': 0, 'wrong': 0}}
    ...
    lines.append("")
    lines.append("🎯 <b>حسب النوع:</b>")
    for qtype, vals in type_stats.items():
        c = vals['correct']
        w = vals['wrong']
        if c + w > 0:
            ratio = c / (c + w) * 100
            lines.append(f"• {qtype}: {c}/{c+w} ({ratio:.0f}%)")
...
async def advanced_filter_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cat in categories:
        ...
        buttons.append([InlineKeyboardButton(
            f"{selected}{cat}",
            callback_data=f"filter_set_cat_{cat}"
        )])
    buttons.append([InlineKeyboardButton(
        f"{'✅ ' if filter_data['category'] is None else ''}جميع الفئات",
        callback_data="filter_set_cat_None"
    )])
    ...
    for label, value in type_options:
        ...
        buttons.append([InlineKeyboardButton(
            f"{selected}{label}",
            callback_data=f"filter_set_type_{value if value else 'None'}"
        )])
    ...
    buttons.append([
        InlineKeyboardButton("✅ تطبيق", callback_data="filter_apply"),
        InlineKeyboardButton("🗑 إلغاء", callback_data="filter_cancel")
    ])
    buttons.append([InlineKeyboardButton("🔙 رجوع", callback_data="menu")])

    await query.edit_message_text(
        "🎯 <b>تصفية متقدمة</b>\n"
        "اختر الفئة ونوع الأسئلة التي تريدها.\n"
...
async def filter_apply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await query.edit_message_text(
            ...
            "حاول تغيير الفئة أو النوع.",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔙 العودة للتصفية", callback_data="advanced_filter")],
    ...
    await query.message.reply_text(
        f"✅ تم العثور على {len(questions)} سؤالاً.",
        parse_mode=ParseMode.HTML
    )
