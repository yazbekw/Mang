...
ومعالجة أسئلة truefalse وتحويلها إلى اختيار من متعدد.
"""
...
# ======================== دالة محسّنة لحساب مؤشر الإجابة ========================
def get_answer_index(options, correct_answer):
    ...
    """
    if isinstance(options, dict):
        # ترتيب المفاتيح (أبجدياً أو عددياً)
        keys = sorted(options.keys(), key=lambda x: int(x) if str(x).isdigit() else str(x))
        # إذا كان correct_answer مفتاحاً، نرجع ترتيبه
        if correct_answer in keys:
            return keys.index(correct_answer)
        # إذا كان correct_answer رقماً، نحاول تحويله إلى مفتاح
        try:
            ...
            pass
        # البحث عن تطابق النص مع القيم (في حال كان correct_answer هو النص نفسه)
...
def load_questions_from_json():
    ...
    # تجميع كل الأسئلة من جميع الملفات في قاموس مؤقت
    all_versions = {}  # المفتاح: نص السؤال، القيمة: قائمة من النسخ (كل نسخة عبارة عن قاموس)
